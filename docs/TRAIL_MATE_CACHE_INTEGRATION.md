...
另外，`TrailMateCenter` 已提供地图内“绘制缓存区域”能力，可直接生成该区域 `0~18` 缩放级别的完整缓存（底图 + 等高线队列），用于避免设备端缩放时出现空白瓦片。
...
- 卫星图源缓存保持 `.jpg`，导出到 SD / U 盘时转码为 `.png`，方便设备端统一 PNG 解码路径。
- 等高线保留透明 `.png`，用于叠加。
...
建议流程（已支持在 UI 中绘制 AOI）：

1. 在 `TrailMateCenter` 地图启用“绘制缓存区域”，用鼠标左键逐点添加目标区域边界，并完成区域。
2. 可选：在“缓存区域列表”中输入名称并保存，形成可复用的区域清单（可删除、可回填到当前区域）。
